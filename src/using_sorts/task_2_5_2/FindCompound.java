...
public class FindCompound {
    ...
    public static String findCompound(ArrayList<String> wordsSorted) {
    ...
    private static int binarySearch(ArrayList<String> wordsSorted, String word, int low, int high) {
        int mid = (low + high)/2;
        if(low > high) {
            return -1;
        }
    ...
    private static int[] getCorrectArray(int sizeArrayList, int[] letters) {
        for(int i = 25, maxPtr = sizeArrayList; i >= 0; i--) {
            if(letters[i] == 1) {
                letters[i] = 0;
                break;
            }
